...
while ( $row = mysqli_fetch_array( $query ) ) {
	...
	<td>
	<?php echo $_SESSION["giohang"][$row["id_sp"]]; ?>
	</td>
	<td>
	<span>
	<?php echo $totalPrice; ?>
...
<td colspan="2"></td>
<td><b><span><?php echo $totalPriceAll; ?></span></b>
</td>
</tr>
...
if ( isset( $_POST[ "submit" ] ) ) {
	...
	if ( isset( $ten ) && isset( $email ) && isset( $sdt ) && isset( $dia_chi ) ) {
		if ( isset( $_SESSION[ "giohang" ] ) ) {
			$arrId = array();
			foreach ( $_SESSION[ "giohang" ] as $id_sp => $sl ) {
				$arrId[] = $id_sp;
		...
		// Phải chỉnh sửa lại
		$mailer->Username = "cuongvm1226@gmail.com"; // GMAIL username
		$mailer->Password = "vmc@@pro00"; // GMAIL password
		$mailer->AddAddress( $email, $ten ); //email người nhận, $email và $ten là 2 biến đc gán bởi $_POST lấy từ trong form
		$mailer->AddCC( "cuongvm1226@gmail.com", "Admin Vietpro Shop" ); // gửi thêm một email cho Admin
		
		// Chuẩn bị gửi thư nào
		$mailer->FromName = 'Vietpro Shop'; // tên người gửi
		$mailer->From = 'cuongvm1226@gmail.com'; // mail người gửi
